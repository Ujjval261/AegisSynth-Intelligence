...
def _safe_generation_config(
    ...
    if "CTGAN" in algorithm or "TVAE" in algorithm:
        ...
        high_card_cols = [
            ...
            if len(safe_data) > 0 and safe_data[col].nunique(dropna=True) > max(250, int(0.85 * len(safe_data)))
        ]
        if high_card_cols:
...
def _select_runtime_algorithm(algorithm: str, safe_data: pd.DataFrame) -> tuple[str, str]:
    ...
    high_card_cols = [
        col for col in safe_data.select_dtypes(include=["object", "category"]).columns
        if rows > 0 and safe_data[col].nunique(dropna=True) > max(250, int(0.85 * rows))
    ]
    if cols > 55 or len(high_card_cols) > 8:
    ...
    if "CTGAN" in algorithm and cols > 35:
